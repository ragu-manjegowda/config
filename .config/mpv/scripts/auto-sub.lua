...
local log = function(string, secs)
...
local download_subs = function(language)
    ...
    if result.status == 0 then
		...
		log('exit code ' .. result.status .. ': ' .. result.stdout .. result.stderr)
        return false
    end
end

-- Check if subtitles should be auto-downloaded:
local autosub_allowed = function()
    local duration = tonumber(mp.get_property('duration'))
    local active_format = mp.get_property('file-format')

    if not bools.auto then
        mp.msg.info('Automatic downloading disabled!')
        return false
...
local should_download_subs_in = function(language)
...
local control_downloads = function()
    ...
    mp.msg.info('Reactivate external subtitle files:')
    mp.commandv('rescan_external_files')
...
mp.register_script_message("download-subs", download_subs)
