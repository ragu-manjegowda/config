...
local function list_update(w, buttons, label, data, objects)
    ...
    for i, o in ipairs(objects) do
        ...
        if text == nil or text == '' then
            ...
            -- naughty.notify({ text = tostring(utf8.len(text_only)) })
            ...
            if has_utf8 and utf8.len(text_only) then
                ...
                text_len = utf8.len(text_only)
                text_end_pos = utf8.offset(text_only, 22)
            else
                text_len = string.len(text_only)
                text_end_pos = 22
            end

            if (text_len > 24) then
                text = text:gsub('>(.-)<', '>' .. string.sub(text_only, 1, text_end_pos - 1) .. '...<')
