...
def get_invitation_from_path(path):
    ...
    begin = raw.find("BEGIN:VCALENDAR")
    if begin == -1:
        raise vobject.base.ParseError("No VCALENDAR found")

    end = raw.rfind("END:VCALENDAR")
    if end == -1:
        raw = raw[begin:] + "\nEND:VCALENDAR\n"
    else:
        raw = raw[begin : end + len("END:VCALENDAR")]

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        return vobject.readOne(io.StringIO(raw), ignoreUnreadable=True)
...
def pretty_print_invitation(invitation):
    ...
    if invitation.method.value == "REPLY":
        ...
        description = (
            event["description"][0].value if "description" in event else "none"
        )
        print("")
        print("\u001b[48m", " " * CONTENT_WIDTH, "\u001b[0m")
        print(
            "\u001b[48;1;37m",
            ("Event Invitation").center(CONTENT_WIDTH),
            "\u001b[0m",
        )
        print(
            "\u001b[48;37m",
            (smart_truncate(title.strip(), 68)).center(CONTENT_WIDTH),
            "\u001b[0m",
        )
        print("\u001b[48m", " " * CONTENT_WIDTH, "\u001b[0m")
        ...
        print(
            ...
            % (
                ...
                or (
                    ...
                    if "Microsoft Teams meeting" in description
                    else "<None>"
                )
            )
        )
